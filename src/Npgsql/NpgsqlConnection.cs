...
public class NpgsqlConnection : DbConnection, ICloneable, IComponent
{
    ...
    internal NpgsqlConnection(NpgsqlDataSource dataSource, NpgsqlConnector connector) : this()
    {
        ...
        Connector = connector;
        connector.Connection = (Npgsql.NpgsqlConnection)this;
    ...
    internal virtual Task Open(bool async, CancellationToken cancellationToken)
    {
        ...
        async Task OpenAsync(bool async, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (enlistToTransaction is not null && _dataSource.TryRentEnlistedPending(enlistToTransaction, (Npgsql.NpgsqlConnection)this, out connector))
                {
                    EnlistedTransaction = enlistToTransaction;
                    enlistToTransaction = null;
                }
                else
                    connector = await _dataSource.Get((Npgsql.NpgsqlConnection)this, timeout, async, cancellationToken);
    ...
    public new virtual NpgsqlCommand CreateCommand()
    {
        ...
        return (NpgsqlCommand)NpgsqlCommand.CreateCachedCommand((Npgsql.NpgsqlConnection)this);
    }
    ...
    public new NpgsqlBatch CreateBatch() => new((Npgsql.NpgsqlConnection)this);
#else
    /// <summary>
    ...
    public override void EnlistTransaction(Transaction? transaction)
    {
        ...
        var volatileResourceManager = new VolatileResourceManager((Npgsql.NpgsqlConnection)this, transaction);
        ...
        ConnectorBindingScope scope, NpgsqlTimeout timeout, bool async, CancellationToken cancellationToken)
    {
        ...
        async ValueTask<NpgsqlConnector> StartBindingScopeAsync()
        {
            try
            {
                ...
                var connector = await _dataSource.Get((Npgsql.NpgsqlConnection)this, timeout, async, cancellationToken);
                Connector = connector;
                connector.Connection = (Npgsql.NpgsqlConnection)this;
    ...
    public override DataTable GetSchema(string? collectionName, string?[]? restrictions)
        => NpgsqlSchema.GetSchema((Npgsql.NpgsqlConnection)this, collectionName, restrictions, async: false).GetAwaiter().GetResult();
    ...
    {
        using (NoSynchronizationContextScope.Enter())
            return NpgsqlSchema.GetSchema((Npgsql.NpgsqlConnection)this, collectionName, restrictions, async: true, cancellationToken);
    }
